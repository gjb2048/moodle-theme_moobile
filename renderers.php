...
/**
 * Renderers for the moobile theme.
 *
 * @package    theme.
...
/**
 * A custom renderer for the moobile theme to produce snippets of content.
 */
// Implemented MDL-33934-2 as appropriate.
class theme_moobile_renderer extends plugin_renderer_base {
     ...
     * @return type
     */
    protected function navigation_node(navigation_node $node, $attrs = null) {
        $items = $node->children;
...
/**
 * Overridden core renderer for the moobile theme
 */
class theme_moobile_core_renderer extends core_renderer {
    ...
    protected function theme_swatch() {
        if (!empty($this->page->theme->settings->colourswatch)) {
            $dtheme = $this->page->theme->settings->colourswatch;
        } else {
            $dtheme = 'a';
        }
        return $dtheme;
    ...
    public function block(block_contents $bc, $region) {
        ...
        $bc = clone($bc);
        // The moobile theme does not support collapsible blocks.
        ...
        $testb = '';
        if (array_key_exists('class', $bc->attributes)) {
            $testb = $bc->attributes['class'];
        }
        $testc = '';
        if (array_key_exists('id', $bc->attributes)) {
    ...
    protected function init_block_hider_js(block_contents $bc) {
        // The moobile theme in no shape or form supports the hiding of blocks
        // this function has been defined and left empty intentionally so that
        // the block hider JS is not even included.
    }

    /**
     * Produces the navigation bar for the moobile theme
     *
     * @return string
     */
    public function navbar() {
        ...
        $navbarcontent .= html_writer::start_tag('select', array('class' => 'navselect', 'data-theme' => $dtheme, 'data-inline' => 'false', 'data-icon' => 'false'));
    ...
    protected function render_help_icon(help_icon $helpicon) {
        ...
        $url = new moodle_url('/help.php', array('component' => $helpicon->component, 'identifier' => $helpicon->identifier, 'lang' => current_language(), 'theme' => 'moobile'));
    ...
    public function notification($message, $classes = 'notifyproblem') {
        return html_writer::tag('div', clean_text($message), array('data-role' => 'none', 'data-icon' => 'alert', 'data-theme' => $this->theme_swatch(), 'class' => renderer_base::prepare_classes($classes)));
    }
...
/**
 * Overridden choice module renderer for the moobile theme
 */
$choice = get_plugin_directory('mod', 'choice');
if (file_exists($choice . '/renderer.php')) {
    global $CFG;
    require_once($CFG->dirroot . '/theme/moobile/renderers/mod_choice_renderer.php');
}
